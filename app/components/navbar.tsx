  ...
  const [toggle, setToggle] = useState(false);
    ...
    return (
        ...
        <nav className="text-lg py-4 px-8 lg:hidden container relative">
          ...
          <Link href="#" className={toggle ? "max-h-60 py-4 px-14 border-t border-slate-300 hover:bg-dark-gradient transition duration-300 ease-out hover:ease-in" : "max-h-0 invisible"} onClick={()=>setToggle(!toggle)}>Services</Link>
          <Link href="#" className={toggle ? "max-h-60 py-4 px-14 border-t border-slate-300 hover:bg-dark-gradient transition duration-300 ease-out hover:ease-in" : "max-h-0 invisible"} onClick={()=>setToggle(!toggle)}>About us</Link>
          <Link href="#" className={toggle ? "max-h-60 py-4 px-14 border-y border-slate-300 hover:bg-dark-gradient transition duration-300 ease-out hover:ease-in" : "max-h-0 invisible"} onClick={()=>setToggle(!toggle)}>About us</Link>
          </div>
